...
## 2.1. Defense-in-Depth (3단계 방어 체계)

> **Cloud Armor 현재 상태:** 승인 대기 중으로 Terraform 코드 주석 처리
...
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ L1: Cloud Armor (GCP LB)                                    │   │
│  │     - WAF 규칙: OWASP Top 10, SQLi, XSS 차단                │   │
│  │     - Rate Limiting: IP당 요청 제한                         │   │
│  │     - 상태: 승인 대기 중 (Terraform 주석 처리)              │   │
│  └────────────────────────┬────────────────────────────────────┘   │
│                           │ 통과                                    │
│                           ▼                                         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ L2: Wazuh (Host OS / Runtime)                               │   │
│  │     - 파일 무결성 모니터링 (FIM)                            │   │
│  │     - 루트킷 탐지, 내부자 위협 방어                         │   │
│  │     - 로그 수집: SQLi, Brute Force 탐지 (Level 12)          │   │
│  └────────────────────────┬────────────────────────────────────┘   │
│                           │ 침해 발생 시                            │
│                           ▼                                         │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │ L3: GCS (Storage)                                           │   │
│  │     - 로그 장기 보관: 180일 (wazuh/), 1년 (ERROR+)          │   │
│  │     - 사후 포렌식 및 증적 관리                              │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────┘
```

### Cloud Armor vs Wazuh 상호 보완

> "Cloud Armor가 다 막아주면, Wazuh는 필요 없나요?"

두 솔루션은 감시하는 레이어가 다릅니다:

| 구분 | Cloud Armor (The Shield) | Wazuh (The Watchtower) |
|------|--------------------------|------------------------|
| **역할** | 최전방 Edge 방어 | 최후의 보루, Host 감시 |
| **차단 대상** | 알려진 공격 패턴, DDoS | 우회 공격, 내부자 위협 |
| **보호 범위** | 인프라 부하 방지 | Post-Exploitation 탐지 |

### 공격 시나리오별 동작 비교

| 공격 유형 | Cloud Armor OFF | Cloud Armor ON |
|-----------|-----------------|----------------|
| **SQLi** | Service-B 도달 → 앱 에러 → **Wazuh 경보** | **LB에서 403** → VM 미도달 |
| **Brute Force** | Nginx 로그 폭증 → **Wazuh Rule 5712** | **429 Rate Limit** → IP 차단 |
| **로그 위치** | `wazuh_logs/`, `npm_data/logs/` | Cloud Logging (LB Level) |
